(() => {
  ...
  // lib/config.js
  var config = {
    apiPhox: "https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api",
    apiUL: "https://webetu.iutnc.univ-lorraine.fr"
  };
  ...
  function loadPicture(idPicture) {
    return __async(this, null, function* () {
      try {
        const rp = yield fetch(config_default.apiPhox + "/photos/" + idPicture);
        if (!rp.ok) {
          throw new Error(`Erreur: ${rp.status}, ${rp.statusText}`);
        }
        const rpFormattee = yield rp.json();
        rpFormattee.photo.url.href = `${config_default.apiUL}${rpFormattee.photo.url.href}`;
        return rpFormattee;
      } catch (error) {
  ...
  function loadResource(uri) {
    return __async(this, null, function* () {
      try {
        const rp = yield fetch(uri);
        if (!rp.ok) {
          throw new Error(`Erreur: ${rp.status}, ${rp.statusText}`);
        }
        return yield rp.json();
      } catch (error) {
  ...
  function displayCategory(category) {
    ...
    htmlCategory.textContent = `cat\xE9gorie : ${category.categorie.nom}`;
  }
  ...
  function load() {
    return __async(this, null, function* () {
      const url = config_default.apiPhox + "/photos";
      const galleryData = yield loadResource(url);
    ...
    return __async(this, null, function* () {
      try {
        if (navigationLinks.next) {
          const url = config_default.apiUL + navigationLinks.next.href;
          const galleryData = yield loadResource(url);
          navigationLinks = galleryData.links;
          return galleryData;
        } else {
    ...
    return __async(this, null, function* () {
      try {
        if (navigationLinks.prev) {
          const url = config_default.apiUL + navigationLinks.prev.href;
          const galleryData = yield loadResource(url);
          navigationLinks = galleryData.links;
          return galleryData;
        } else {
    ...
    return __async(this, null, function* () {
      try {
        if (navigationLinks.first) {
          const url = config_default.apiUL + navigationLinks.first.href;
          const galleryData = yield loadResource(url);
          navigationLinks = galleryData.links;
          return galleryData;
        } else {
    ...
    return __async(this, null, function* () {
      try {
        if (navigationLinks.last) {
          const url = config_default.apiUL + navigationLinks.last.href;
          const galleryData = yield loadResource(url);
          navigationLinks = galleryData.links;
          return galleryData;
        } else {
  ...
  function display_gallerie(galerie) {
    const htmlGallerie = document.querySelector("#galleryTemplate").innerHTML;
    const template = import_handlebars2.default.compile(htmlGallerie);
    galerie.photos = galerie.photos.map((photo) => {
      photo.photo.thumbnail.href = config_default.apiUL + photo.photo.thumbnail.href;
      return photo;
    });
  ...
  function getPicture(id) {
    return __async(this, null, function* () {
      try {
        const data = yield loadPicture(id);
        const category = yield getCategory(data);
        const comments = yield getComments(data);
        displayPicture(data);
  ...
  function getCategory(image) {
    return __async(this, null, function* () {
      const url = config_default.apiUL + image.links.categorie.href;
      return yield loadResource(url);
    });
  }
  function getComments(image) {
    return __async(this, null, function* () {
      const url = config_default.apiUL + image.links.comments.href;
      return yield loadResource(url);
    });
  }
  document.addEventListener("DOMContentLoaded", () => {
    ...
    afficherGalerieBtn.addEventListener("click", function() {
      return __async(this, null, function* () {
    ...
    boutonNavigation.addEventListener("click", (e) => __async(void 0, null, function* () {
